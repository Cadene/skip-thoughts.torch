...
class SequentialDropout(torch.nn.Module):
    ...
    def _make_noise(self, input):
        return torch.autograd.Variable(input.data.new().resize_as_(input.data))
...
if __name__ == '__main__':
    ...
    dp = SequentialDropout(p=0.5, training=True)
    input = torch.autograd.Variable(torch.ones(1,10), volatile=True)
